...
void OpenGLMatrixManager::rotate( double a, double x, double y, double z )
{
    ...
    double m[16] = {x * x * ( 1 - c ) + c, y * x * ( 1 - c ) + z * s, x * z * ( 1 - c ) - y * s, 0,
...
void OpenGLMatrixManager::lookAt( double eyeX, double eyeY, double eyeZ, double centerX, double centerY, double centerZ, double upX, double upY, double upZ )
{
    ...
    //Montando a matriz M.
    double m[16] = {( double ) sX, ( double ) uX, -( double ) fX, 0.0f,
...
void OpenGLMatrixManager::frustum( double left, double right, double bottom, double top, double nearVal, double farVal )
{
    ...
    double f[16] = {2 * nearVal / ( right - left ), 0, 0, 0,
...
void OpenGLMatrixManager::ortho( double left, double right, double bottom, double top, double nearVal, double farVal )
{
    ...
    double o[16] = {2 / ( right - left ), 0, 0, 0,
                    0, 2 / ( top - bottom ), 0, 0,
...
void OpenGLMatrixManager::perspective( double fovY, double aspect, double zNear, double zFar )
{
    ...
    double p[16] = {( double ) ( f / aspect ), 0.0f, 0.0f, 0.0f,

...
//                    double eye[3] = {2, 2, 3 };
    ...
    for (unsigned int i = 0; i < numSpheres; i++) {
        for (unsigned int j = 0; j < numSpheres; j++) {
            ...
            _surface[0]._shader->setTangentAndBitangent(&_surface[0]._tangent[0], &_surface[0]._bitangent[0]);
...
void MainWindow::generateTangentsAndBitangents() {
    _surface[0]._tangent.resize(_surface[0]._vertex.size());
    _surface[0]._bitangent.resize(_surface[0]._vertex.size());


    for (unsigned int i = 0; i < _surface[0]._triangles.size()/3; i++) {

        unsigned int id1 = _surface[0]._triangles[3 * i + 0];
        unsigned int id2 = _surface[0]._triangles[3 * i + 1];
        unsigned int id3 = _surface[0]._triangles[3 * i + 2];


        double v0[3] = {
            _surface[0]._vertex[ 3 * id1 ],
            _surface[0]._vertex[ 3 * id1 + 1 ],
            _surface[0]._vertex[ 3 * id1 + 2 ]
        };

        double v1[3] = {
            _surface[0]._vertex[ 3 * id2 ],
            _surface[0]._vertex[ 3 * id2 + 1 ],
            _surface[0]._vertex[ 3 * id2 + 2 ]
        };

        double v2[3] = {
            _surface[0]._vertex[ 3 * id3 ],
            _surface[0]._vertex[ 3 * id3 + 1 ],
            _surface[0]._vertex[ 3 * id3 + 2 ]
        };


        double uv0[2] = {_surface[0]._texCoord[ 2 * id1 ], _surface[0]._texCoord[ 2 * id1 + 1]};
        double uv1[2] = {_surface[0]._texCoord[ 2 * id2 ], _surface[0]._texCoord[ 2 * id2 + 1]};
        double uv2[2] = {_surface[0]._texCoord[ 2 * id3 ], _surface[0]._texCoord[ 2 * id3 + 1]};

        double deltaPos1[3] = {v1[0] - v0[0],
            v1[1] - v0[1],
            v1[2] - v0[2]};

        double deltaPos2[3] = {v2[0] - v0[0],
            v2[1] - v0[1],
            v2[2] - v0[2]};

        double deltaUV1[2] = {uv1[0] - uv0[0],
            uv1[1] - uv0[1]};

        double deltaUV2[2] = {uv2[0] - uv0[0],
            uv2[1] - uv0[1]};
        ...
        double tangent[3] = {(deltaPos1[0] * deltaUV2[1] - deltaPos2[0] * deltaUV1[1]) * r,
            (deltaPos1[1] * deltaUV2[1] - deltaPos2[1] * deltaUV1[1]) * r,
            (deltaPos1[2] * deltaUV2[1] - deltaPos2[2] * deltaUV1[1]) * r};

        double bitangent[3] = {(deltaPos2[0] * deltaUV1[0] - deltaPos1[0] * deltaUV2[0]) * r,
            (deltaPos2[1] * deltaUV1[0] - deltaPos1[1] * deltaUV2[0]) * r,
            (deltaPos2[2] * deltaUV1[0] - deltaPos1[2] * deltaUV2[0]) * r};

        _surface[0]._tangent[3 * id1 + 0] += tangent[0];
        _surface[0]._tangent[3 * id1 + 1] += tangent[1];
        _surface[0]._tangent[3 * id1 + 2] += tangent[2];

        _surface[0]._tangent[3 * id2 + 0] += tangent[0];
        _surface[0]._tangent[3 * id2 + 1] += tangent[1];
        _surface[0]._tangent[3 * id2 + 2] += tangent[2];

        _surface[0]._tangent[3 * id3 + 0] += tangent[0];
        _surface[0]._tangent[3 * id3 + 1] += tangent[1];
        _surface[0]._tangent[3 * id3 + 2] += tangent[2];


        _surface[0]._bitangent[3 * id1 + 0] += bitangent[0];
        _surface[0]._bitangent[3 * id1 + 1] += bitangent[1];
        _surface[0]._bitangent[3 * id1 + 2] += bitangent[2];

        _surface[0]._bitangent[3 * id2 + 0] += bitangent[0];
        _surface[0]._bitangent[3 * id2 + 1] += bitangent[1];
        _surface[0]._bitangent[3 * id2 + 2] += bitangent[2];

        _surface[0]._bitangent[3 * id3 + 0] += bitangent[0];
        _surface[0]._bitangent[3 * id3 + 1] += bitangent[1];
        _surface[0]._bitangent[3 * id3 + 2] += bitangent[2];
